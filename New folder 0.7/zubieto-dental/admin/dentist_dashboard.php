...
// 1.  Today's Schedule query now includes `a.status`
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
    ...
    <div class="btn-toolbar mb-2 mb-md-0"><a href="appointments.php" class="btn btn-sm btn-outline-secondary">View Full Calendar</a></div>
</div>

<div class="alert alert-info">Welcome, <strong><?php echo htmlspecialchars($_SESSION['full_name']); ?></strong>!</div>

<!-- KPI Cards -->
<div class="row">
    <div class="col-md-4"><div class="card text-white bg-primary mb-3"><div class="card-body d-flex justify-content-between align-items-center"><div><h5 class="card-title">My Appointments Today</h5><h2 class="card-text"><?php echo $today_count; ?></h2></div><i class="fas fa-calendar-day fa-3x opacity-75"></i></div></div></div>
    <div class="col-md-4"><div class="card text-white bg-warning mb-3"><div class="card-body d-flex justify-content-between align-items-center"><div><h5 class="card-title">Branch Follow-ups</h5><h2 class="card-text"><?php echo $followup_count; ?></h2></div><i class="fas fa-user-clock fa-3x opacity-75"></i></div></div></div>
    <div class="col-md-4"><div class="card text-white bg-success mb-3"><div class="card-body d-flex justify-content-between align-items-center"><div><h5 class="card-title">My Completed (Month)</h5><h2 class="card-text"><?php echo $completed_count; ?></h2></div><i class="fas fa-check-circle fa-3x opacity-75"></i></div></div></div>
</div>
...
<div class="row">
    ...
    <div class="col-lg-7">
        ...
        <div class="card"><div class="card-body"><div class="table-responsive"><table class="table table-hover">
            ...
            <thead class="table-light">
                <tr><th>Time</th><th>Patient</th><th>Service / Reason</th><th class="text-center">Status</th><th class="text-center">Action</th></tr>
            </thead>
            <tbody>
                <?php if ($todays_schedule_result && $todays_schedule_result->num_rows > 0): ?>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <?php
                                    ...
                                    switch ($status) {
                                        case 'Scheduled': $badge_class = 'bg-primary'; break;
                                        case 'Completed': $badge_class = 'bg-success'; break;
                                        case 'Cancelled': $badge_class = 'bg-secondary'; break;
                                        case 'No-Show': $badge_class = 'bg-danger'; break;
                                    }
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <!-- **CHANGED**: Colspan updated to 5 -->
                    <tr><td colspan="5"><div class="alert alert-light text-center mb-0">No appointments scheduled for you today.</div></td></tr>
                <?php endif; ?>
            </tbody>
        </table></div></div></div>
    </div>
    ...
    <div class="col-lg-5">
        ...
        <div class="card"><div class="card-body"><div class="table-responsive"><table class="table table-hover"><thead class="table-light"><tr><th>Date</th><th>Patient</th><th>Reason</th></tr></thead><tbody>
            <?php if ($pending_followups_result && $pending_followups_result->num_rows > 0): ?>
                <?php while($followup = $pending_followups_result->fetch_assoc()): ?>
                    <tr><td><?php echo date('M d, Y', strtotime($followup['next_appt'])); ?></td><td><a href="patient_view.php?id=<?php echo $followup['patient_id']; ?>"><?php echo htmlspecialchars($followup['last_name'] . ', ' . $followup['first_name']); ?></a></td><td><?php echo htmlspecialchars($followup['procedure_done']); ?></td></tr>
                <?php endwhile; ?>
            <?php else: ?>
