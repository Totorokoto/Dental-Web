<?php
// FILE: admin/patient_add_process.php (MODIFIED)
...
try {
    ...
    $sql_patient = "INSERT INTO patients (last_name, first_name, middle_name, nickname, birthdate, age, gender, civil_status, nationality, religion, occupation, address, mobile_no, parent_guardian_name, parent_guardian_occupation, chief_complaint, history_of_present_illness, previous_dentist, last_dental_visit, procedures_done_perio, procedures_done_resto, procedures_done_os, procedures_done_prostho, procedures_done_endo, procedures_done_ortho, procedures_done_others_specify, complications, physician_name, physician_address, physician_phone, last_physical_exam, blood_pressure, respiratory_rate, pulse_rate, temperature) VALUES ('$last_name', '$first_name', '$middle_name', '$nickname', '$birthdate', $age, '$gender', '$civil_status', '$nationality', '$religion', '$occupation', '$address', '$mobile_no', '$parent_guardian_name', '$parent_guardian_occupation', '$chief_complaint', '$history_of_present_illness', '$previous_dentist', $last_dental_visit, ".sanitize_bool('procedures_done_perio').", ".sanitize_bool('procedures_done_resto').", ".sanitize_bool('procedures_done_os').", ".sanitize_bool('procedures_done_prostho').", ".sanitize_bool('procedures_done_endo').", ".sanitize_bool('procedures_done_ortho').", '$procedures_done_others_specify', '$complications', '$physician_name', '$physician_address', '$physician_phone', $last_physical_exam, '$blood_pressure', '$respiratory_rate', '$pulse_rate', '$temperature')";
    if (!$conn->query($sql_patient)) { throw new Exception("Error creating patient record: " . $conn->error); }
    ...
    $other_conditions_to_know = $conn->real_escape_string($_POST['other_conditions_to_know']); // New field
    ...
    if (!$conn->query($sql_history)) { throw new Exception("Error creating medical history: " . $conn->error); }
