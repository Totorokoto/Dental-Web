<?php
// FILE: admin/appointments.php (FINAL VERSION WITH ADD BUTTON)
require 'includes/header.php';
require '../includes/db_connect.php';

// Fetch patients and dentists for dropdown menus
...
<!-- Page Header with NEW Add Appointment Button -->
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Appointment Calendar</h1>
    <!-- ========================================================== -->
    <!-- NEW: ADD NEW APPOINTMENT BUTTON ADDED HERE                 -->
    <!-- ========================================================== -->
...
<div class="modal fade" id="appointmentModal" tabindex="-1" aria-labelledby="appointmentModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title" id="appointmentModalLabel">Manage Appointment</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
            <form id="appointmentForm">
                <div class="modal-body">
                    <input type="hidden" name="appointment_id" id="appointment_id">
                    <div class="mb-3"><label for="patient_id" class="form-label"><i class="fas fa-user me-2"></i>Patient <span class="text-danger">*</span></label><select class="form-select searchable-select" id="patient_id" name="patient_id" required style="width: 100%;"><option value="">Search for a patient...</option><?php foreach ($patients as $patient): ?><option value="<?php echo $patient['patient_id']; ?>"><?php echo htmlspecialchars($patient['last_name'] . ', ' . $patient['first_name']); ?></option><?php endforeach; ?></select></div>
                    <div class="mb-3"><label for="dentist_id" class="form-label"><i class="fas fa-user-md me-2"></i>Dentist <span class="text-danger">*</span></label><select class="form-select" id="dentist_id" name="dentist_id" required><option value="">Select a Dentist...</option><?php foreach ($dentists as $dentist): ?><option value="<?php echo $dentist['user_id']; ?>"><?php echo htmlspecialchars($dentist['full_name']); ?></option><?php endforeach; ?></select></div>
                    <div class="mb-3"><label for="appointment_date" class="form-label"><i class="fas fa-clock me-2"></i>Date & Time <span class="text-danger">*</span></label><input type="datetime-local" class="form-control" id="appointment_date" name="appointment_date" required></div>
                    <div class="mb-3"><label for="service_description" class="form-label"><i class="fas fa-notes-medical me-2"></i>Service / Reason <span class="text-danger">*</span></label><textarea class="form-control" id="service_description" name="service_description" rows="3" required></textarea></div>
                    <div class="mb-3"><label for="status" class="form-label"><i class="fas fa-info-circle me-2"></i>Status</label><select class="form-select" id="status" name="status"><option value="Scheduled">Scheduled (Blue)</option><option value="Completed">Completed (Green)</option><option value="Cancelled">Cancelled (Gray)</option><option value="No-Show">No-Show (Red)</option></select></div>
                </div>
                <div class="modal-footer"><button type="button" class="btn btn-danger me-auto" id="deleteAppointmentBtn"><i class="fas fa-trash me-2"></i>Delete</button><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button><button type="submit" class="btn btn-primary"><i class="fas fa-save me-2"></i>Save Changes</button></div>
            </form>
        </div>
    </div>
</div>
...
<!-- 5. Custom Script for this page's functionality -->
<script>
$(document).ready(function() {
        ...
        eventDrop: function(info) {
            if (confirm("Are you sure you want to move this appointment?")) {
                 $.post('appointment_edit_process.php', {
                    ...
                    // Always refetch events after a drop
    ...
    // ====================================================================
    // THE FIX IS HERE: A robust jQuery handler for the form submission.
    // ====================================================================
    form.on('submit', function(e) {
        ...
        $.post(url, formData, function(response) {
            ...
            if(response.success) {
                ...
                // *** THIS IS THE CRITICAL LINE THAT REFRESHES THE CALENDAR ***
                calendar.refetchEvents();
            }
        }, 'json');
    });
    ...
    $('#deleteAppointmentBtn').on('click', function() {
        ...
        if(appointmentId && confirm('Are you sure you want to permanently delete this appointment?')) {
            $.post('appointment_delete_process.php', { appointment_id: appointmentId }, function(response) {
                ...
                if(response.success) {
                    appointmentModal.hide();
                    // Also refetch events after a successful delete
                    calendar.refetchEvents();
                }
            }, 'json');
        }
...
<?php
$conn->close();
// We no longer require footer.php as all scripts are self-contained on this page
?>