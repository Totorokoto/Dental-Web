...
// Fetch active dentists/admins from the patient's branch for the dropdowns
$dentists_list = [];
$patient_branch = $patient['branch'];
...
if ($stmt_dentists = $conn->prepare($sql_dentists)) {
    $stmt_dentists->bind_param("s", $patient_branch);
    $stmt_dentists->execute();
    $result_dentists = $stmt_dentists->get_result();
    while ($row = $result_dentists->fetch_assoc()) {
        $dentists_list[] = $row;
    }
    $stmt_dentists->close();
}
$dentist_names_array = array_column($dentists_list, 'full_name');

// Helper functions
function check_radio($value, $expected) { if (isset($value) && $value == $expected) { echo 'checked'; } }
function check_box($value) { if (!empty($value) && $value == 1) { echo 'checked'; } }
?>

<!-- Custom styles -->
<style>
    .form-section-card { border-radius: 1rem; box-shadow: var(--card-shadow); overflow: hidden; }
                ...
                <div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" data-bs-parent="#patientEditAccordion">
                  <div class="accordion-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-3"><label class="form-label">Birthdate <span class="text-danger">*</span></label><input type="text" class="form-control date-picker-past" id="birthdate" name="birthdate" value="<?php echo $patient['birthdate']; ?>" placeholder="YYYY-MM-DD" required> <span id="ageDisplay" class="text-muted small ms-2"></span></div>
                        ...
                        <div class="col-md-4"><label class="form-label">Mobile # <span class="text-danger">*</span></label><input type="tel" class="form-control" name="mobile_no" value="<?php echo htmlspecialchars($patient['mobile_no']); ?>" required maxlength="13"></div>
                        ...
                        <div class="col-md-6"><label class="form-label">Email Address</label><input type="email" class="form-control" name="email" value="<?php echo htmlspecialchars($patient['email']); ?>"></div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="accordion-body">
                 <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="previous_dentist">
                             <option value="" <?php if(empty($patient['previous_dentist'])) echo 'selected'; ?>>None / Not Applicable</option>
                            <?php foreach($dentists_list as $dentist): ?>
                                <option value="<?php echo htmlspecialchars($dentist['full_name']); ?>" <?php if($patient['previous_dentist'] == $dentist['full_name']) echo 'selected'; ?>>
                                    <?php echo htmlspecialchars($dentist['full_name']); ?>
                                </option>
                            <?php endforeach; ?>
                            <option value="Other / Not Listed" <?php if (!in_array($patient['previous_dentist'], $dentist_names_array) && !empty($patient['previous_dentist'])) echo 'selected'; ?>>Other / Not Listed</option>
                        </select>
                    </div>

                    <div class="col-md-6"><label class="form-label">Last Dental Visit</label><input type="text" class="form-control date-picker-past" name="last_dental_visit" value="<?php echo $patient['last_dental_visit']; ?>" placeholder="YYYY-MM-DD"></div>
              ...
              <div class="accordion-body">
                <div class="row g-3">
                    <div class="col-md-5">
                        ...
                        <select class="form-select" name="physician_name">
                            <option value="" <?php if(empty($patient['physician_name'])) echo 'selected'; ?>>None / Not Applicable</option>
                            <?php foreach($dentists_list as $dentist): ?>
                                <option value="<?php echo htmlspecialchars($dentist['full_name']); ?>" <?php if($patient['physician_name'] == $dentist['full_name']) echo 'selected'; ?>>
                                    <?php echo htmlspecialchars($dentist['full_name']); ?>
                                </option>
                            <?php endforeach; ?>
                            <option value="Other" <?php if (!in_array($patient['physician_name'], $dentist_names_array) && !empty($patient['physician_name'])) echo 'selected'; ?>>Other (Not Listed)</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-3"><label class="form-label">Phone Number</label><input type="tel" class="form-control" name="physician_phone" value="<?php echo htmlspecialchars($patient['physician_phone']); ?>" maxlength="13"></div>
                    <div class="col-md-12"><label class="form-label">Date of Last Physical Examination</label><input type="text" class="form-control date-picker-past" name="last_physical_exam" value="<?php echo $patient['last_physical_exam']; ?>" placeholder="YYYY-MM-DD"></div>
                    <hr class="my-4">
                    ...
                    <div class="row row-cols-1 row-cols-sm-2 row-cols-lg-3 g-2">
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_high_blood_pressure" value="1" <?php check_box($patient['has_high_blood_pressure']); ?>><label class="form-check-label">High Blood Pressure</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_low_blood_pressure" value="1" <?php check_box($patient['has_low_blood_pressure']); ?>><label class="form-check-label">Low Blood Pressure</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_epilepsy_convulsions" value="1" <?php check_box($patient['has_epilepsy_convulsions']); ?>><label class="form-check-label">Epilepsy</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_aids_hiv" value="1" <?php check_box($patient['has_aids_hiv']); ?>><label class="form-check-label">AIDS or HIV</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_stomach_troubles_ulcer" value="1" <?php check_box($patient['has_stomach_troubles_ulcer']); ?>><label class="form-check-label">Stomach Troubles</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_fainting_seizure" value="1" <?php check_box($patient['has_fainting_seizure']); ?>><label class="form-check-label">Fainting Seizure</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_rapid_weight_loss" value="1" <?php check_box($patient['has_rapid_weight_loss']); ?>><label class="form-check-label">Rapid Weight Loss</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_hepatitis_jaundice" value="1" <?php check_box($patient['has_hepatitis_jaundice']); ?>><label class="form-check-label">Hepatitis/Jaundice</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_tuberculosis" value="1" <?php check_box($patient['has_tuberculosis']); ?>><label class="form-check-label">Tuberculosis</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_swollen_ankles" value="1" <?php check_box($patient['has_swollen_ankles']); ?>><label class="form-check-label">Swollen Ankles</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_kidney_disease" value="1" <?php check_box($patient['has_kidney_disease']); ?>><label class="form-check-label">Kidney Disease</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_diabetes" value="1" <?php check_box($patient['has_diabetes']); ?>><label class="form-check-label">Diabetes</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_bleeding_blood_disease" value="1" <?php check_box($patient['has_bleeding_blood_disease']); ?>><label class="form-check-label">Bleeding/Blood Disease</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_cancer_tumor" value="1" <?php check_box($patient['has_cancer_tumor']); ?>><label class="form-check-label">Cancer/Tumor</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_rheumatic_fever_disease" value="1" <?php check_box($patient['has_rheumatic_fever_disease']); ?>><label class="form-check-label">Rheumatic Fever</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_hay_fever_allergies" value="1" <?php check_box($patient['has_hay_fever_allergies']); ?>><label class="form-check-label">Hay Fever</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_respiratory_problems" value="1" <?php check_box($patient['has_respiratory_problems']); ?>><label class="form-check-label">Respiratory Problems</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="had_stroke" value="1" <?php check_box($patient['had_stroke']); ?>><label class="form-check-label">Stroke</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_chest_pain" value="1" <?php check_box($patient['has_chest_pain']); ?>><label class="form-check-label">Chest Pain</label></div></div>
                        <div class="col"><div class="form-check"><input class="form-check-input" type="checkbox" name="has_asthma" value="1" <?php check_box($patient['has_asthma']); ?>><label class="form-check-label">Asthma</label></div></div>
                    </div>
